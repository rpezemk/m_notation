...
def paint_time_holder(q_painter: QPainter, v_n: VisualNote, v_note_spacing: int, base_y_offset: int):
    if isinstance(v_n.inner, Note):
        paint_visual_note(q_painter, v_n, v_note_spacing, base_y_offset)
    else:
        paint_visual_rest(q_painter, v_n, v_note_spacing, base_y_offset)
        
def paint_visual_note(q_painter: QPainter, v_n: VisualNote, v_note_spacing: int, base_y_offset: int):
    inner = v_n.inner
    ...
    maybe = [p for p in note_painters if p.t == inner_type and p.d == inner.base_duration and p.orientation_up == is_up]
...
def paint_visual_rest(q_painter: QPainter, v_n: VisualNote, v_note_spacing: int, base_y_offset: int):
    inner = v_n.inner
    inner_type = type(inner)
    ...
    maybe = [p for p in rest_painters if p.t == inner_type and p.d == inner.base_duration and p.orientation_up == is_up]
